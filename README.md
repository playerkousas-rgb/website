...
- 同分按名稱排序；排行榜分數達 100 才顯示數字；「最新上架」按上架時間由新至舊排列
...
| 排行榜指標（只在排行榜） | 🔥／⭐／❤️／🆕 | 🔥 最多人點擊／⭐ 最多人收藏／❤️ 最受歡迎／🆕 最新上架 |
...
- 頂部「回報/意見」會在本站打開表格，不會跳去通告圖書館；表單 payload 對齊 Scout Admin 官方 `widget.js`：問題回報分開送標題／詳情／嚴重度，意見回饋支援建議／讚／批評／其他，來源固定為 `SCOUT APP STORE`；旅團號、姓名、聯絡方式均可選填，並送到官方 Apps Script 接收端。詳細規格見 [Scout Admin 對接說明](https://github.com/playerkousas-rgb/scout-admin/blob/main/SCOUT_ADMIN.md)。
